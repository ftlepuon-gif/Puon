...
local function runAutoSteal()
    ...
    if floor == 1 then
        ...
        if not carpetTp(targetPos, true) then
            if stealSession == mySession then
                fullStop("Teleport failed — please retry", Color3.fromRGB(255, 80, 80), true)
            end
            return
        end
        if not alive() then isWorking = false return end
        ...
        if stealStatusLabel then
            stealStatusLabel.Text       = "Floor 1  ·  Stabilizing position..."
    ...
    if not alive() then isWorking = false return end
    if stealStatusLabel then
        stealStatusLabel.Text       = "TP Process  ·  Locking altitude..."
    ...
    if not root then isWorking = false return end
    if stealStatusLabel then
        stealStatusLabel.Text       = "Step 3  ·  Positioning over target..."
        stealStatusLabel.TextColor3 = Color3.fromRGB(180, 180, 180)
    end
    local protection = addProtection()
    ...
    if stealStatusLabel then
        stealStatusLabel.Text       = "Step 3  ·  Stabilizing position..."
